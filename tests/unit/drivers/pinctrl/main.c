...
/*
 * Switch off logging.
 * Pretend log.h to be already included.
 *
 * Logging does not support 64bit pointers that are
 * usual for the host platform.
 */
...
/* Workaround:
 * ztest sets CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC
 */
#undef CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC
 ...
 * cogeno.out_include('drivers/pinctrl.tmpl.c')
 * @endcode{.cogeno.py}
 */
 ...
 * @code{.cogeno.py}
 * test_device_id = cogeno.edts().get_device_ids_by_compatible(compatible)[0]
 * test_device = cogeno.edts().get_device_by_device_id(test_device_id)
 * cogeno.outl('#define TEST_DRIVER_NAME "{}"'.format( \
 *     test_device.get_property('label')))
 * cogeno.outl('#define TEST_PIN_COUNT {}'.format( \
 *     _pin_controllers[0].pin_count()))
 * cogeno.outl('#define TEST_FUNCTION_COUNT {}'.format( \
 *     _pin_controllers[0].function_count()))
 * cogeno.outl('#define TEST_STATE_NAME_COUNT {}'.format( \
 *     _pin_controllers[0].state_name_count()))
 * cogeno.outl('#define TEST_STATE_COUNT {}'.format( \
 *     _pin_controllers[0].state_count()))
 * cogeno.outl('#define TEST_PINCTRL_COUNT {}'.format( \
 *     _pin_controllers[0].pinctrl_count()))
 *
 * test_client_name = "UART_1"
 * test_client_device_id = cogeno.edts().get_device_id_by_label(test_client_name)
 * test_client = cogeno.edts().get_device_by_device_id(test_client_device_id)
 * cogeno.outl('#define TEST_CLIENT_NAME "{}"'.format(test_client_name))
 * cogeno.outl('#define TEST_CLIENT_FUNCTION {}'.format( \
 *     _pin_controllers[0].function_id(test_client_name)))
 * cogeno.outl('#define TEST_CLIENT_DEFAULT_STATE {}'.format( \
