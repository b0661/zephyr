...
class EDTSExtractorMixin(object):
    ...
    def _extract_property(self, node_address, prop):
        ...
        if node_address.endswith('/partitions'):
            # special case:
            # partition groups have a compatible but are no device
            # and are handled by the flash node.
            self.extractors().extract_flash(node_address, prop)
        elif prop == 'reg':
    ...
    def _extract_node(self, root_node_address, sub_node_address,
        ...
        if node_compat not in bindings:
            # we do not extract nodes without a (known) compatible
            return
                ...
                for node_address in dts.reduced:
                    if root_node_address + '/' in node_address:
                        ...
                        self._extract_node(root_node_address, node_address,
                                           binding_prop_value)
            # We ignore 'generation' as we do not create defines
