...
class DTHeuristics(DTDirective):
    ...
    def _edts_insert_device_parent_child_relation(self, node_address):
        ...
        parent_node_address = node_address
        while True:
            parent_node_address = dts.get_parent_address(parent_node_address)
            if parent_node_address == '':
                # no more parents
                break
            compatibles = dts.get_compats(parent_node_address)
            if compatibles:
                # there is a parent device,
                if 'simple-bus' in compatibles:
                    # only use the ones that have a minimum control on the child
                    continue
                if parent_node_address.endswith('/partitions'):
                    # partitions group belongs to flash node - ignore
                    continue
                if ('pin-controller' in parent_node_address) and \
                    (dts.get_node_compat(node_address) is None):
