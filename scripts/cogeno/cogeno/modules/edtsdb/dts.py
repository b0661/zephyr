...
class DTS(DTSPHandlesMixin, DTSCompatiblesMixin, DTSAliasesMixin,
    ...
    ##
    # @brief Get node adresses of node's children.
    #
    # @return list of addresses, maybe empty
    def get_children_addresses(self, node_address):
        children_addresses = []
        for subnode_address in self.reduced.keys():
            if self.get_parent_address(subnode_address) == node_address:
                children_addresses.append(subnode_address)
