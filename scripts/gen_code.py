...
if cogeno_module_dir is None:
    ...
    if cogeno_spec is None:
        ...
        if isinstance(cogeno_spec.submodule_search_locations, list):
            # We have a list of search pathes
            cogeno_module_dir = Path(cogeno_spec.submodule_search_locations[0])
        else:
            cogeno_module_dir = Path(cogeno_spec.submodule_search_locations._path[0])
        if 'scripts/cogeno' in str(cogeno_module_dir):
            # 3. - use cogeno copy in scripts directory
            from cogeno.cogeno.cogeno import main
            cogeno_module_dir = Path(__file__).resolve().parent.joinpath('cogeno/cogeno')
            print("NOTE: you are running a copy of cogeno from '{}';".
                format(cogeno_module_dir),
            ...
            from cogeno.cogeno import main
            print("NOTE: you are running cogeno from a local installation at {};".
                  format(cogeno_module_dir))
