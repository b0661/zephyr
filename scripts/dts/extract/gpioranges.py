...
class DTGpioRanges(DTDirective):
    ...
    def extract(self, node_address, yaml, y_key, names, defs, def_label):
        ...
        gpio_ranges = reduced[node_address]['props'][y_key]
        ...
        gpio_label = get_node_label_string(node_address)
        ...
        for p in prop_list:
            ...
            if gpio_range_cell < gpio_range_cells:
                ...
                range_label = get_label_string(range_label_prefix + ["CLIENT"])
                prop_def[range_label] = gpio_label
                # - gpio pin number of base pin of GPIO port
                range_label = get_label_string(range_label_prefix + \
                                               ["CLIENT", "BASE"])
                ...
                range_label = get_label_string(range_label_prefix + ["NPINS"])
                ...
                ###############################
                index_label = get_label_string([gpio_label, \
                                                "GPIO", "RANGE", "COUNT"])
                index = self._data.get(index_label, 0)
                self._data[index_label] = index + 1
                ...
                # - gpio pin number of base pin of GPIO port
                range_label = get_label_string(range_label_prefix + ["BASE"])
                prop_def[range_label] = str(gpio_pin_start)
                # - pin controller pin number of base pin of GPIO port
                range_label = get_label_string(range_label_prefix + \
                                               ["CONTROLLER", "BASE"])
