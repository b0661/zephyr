...
class EDTSExtractorMixin(object):
    ...
    def _extract_property(self, node_address, prop):
        ...
        if prop == 'reg':
            ...
            extractors.extract_gpioranges(node_address, prop)
            ...
            extractors.extract_gpios(node_address, prop)
            ...
            if 'properties' in binding_prop_value:
                for node_address in dts.reduced:
                    if root_node_address + '/' in node_address:
                        self._extract_node(dts, bindings, root_node_address,
                                           node_address, binding_prop_value)
    ...
    def _extract_specification(self):
        ...
        for node_address in dts.reduced:
            node_compat = dts.get_node_compat(node_address)
            ...
            if node_compat is None:
                ...
                node_compat = dts.get_parent_compat(node_address)
                if node_compat is not None and node_compat in bindings:
                    self._extract_node(node_address, node_address, None)
    ...
    # @param bindings_dir_pathes YAML file directories, we allow multiple
    #
    def extract(self, dts_file_path, bindings_dir_pathes):

        dts = DTS(dts_file_path)

        # Extract bindings of all compatibles
        bindings = Bindings.bindings(dts.compatibles, bindings_dir_pathes)
        if bindings == {}:
