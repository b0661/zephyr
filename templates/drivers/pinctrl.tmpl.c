 ...
 *
 * _pin_controllers = cogeno.edts().get_devices_by_compatible(compatible)
 *
 * cogeno.outl("/" + "* {} x '{}' pin controller devices *" \
...
LOG_MODULE_REGISTER(pinctrl);

/** Define template types for minimum footprint.
 * @code{.cogeno.py}
 *
 * if _pin_controllers[0].max_pin_count() < 256:
 *     cogeno.outl("typedef u8_t pinctrl_tmpl_pin_t;")
 * else:
 *     cogeno.outl("typedef u16_t pinctrl_tmpl_pin_t;")
 * if _pin_controllers[0].max_function_count() < 256:
 *     cogeno.outl("typedef u8_t pinctrl_tmpl_mux_t;")
 *     cogeno.outl("typedef u8_t pinctrl_tmpl_function_id_t;")
 * else:
 *     cogeno.outl("typedef u16_t pinctrl_tmpl_mux_t;")
 *     cogeno.outl("typedef u16_t pinctrl_tmpl_function_id_t;")
 * if _pin_controllers[0].max_state_name_count() < 256:
 *     cogeno.outl("typedef u8_t pinctrl_tmpl_state_name_id_t;")
 * else:
 *     cogeno.outl("typedef u16_t pinctrl_tmpl_state_name_id_t;")
 * if _pin_controllers[0].max_state_count() < 256:
 *     cogeno.outl("typedef u8_t pinctrl_tmpl_state_id_t;")
 * else:
 *     cogeno.outl("typedef u16_t pinctrl_tmpl_state_id_t;")
 * if _pin_controllers[0].max_pinctrl_count() < 256:
 *     cogeno.outl("typedef u8_t pinctrl_tmpl_pinctrl_id_t;")
 * else:
 *     cogeno.outl("typedef u16_t pinctrl_tmpl_pinctrl_id_t;")
 * @endcode{.cogeno.py}
 */
/** @code{.cogeno.ins}@endcode */
					      ...
					      u8_t *num_pins)
{
	...
	for (u8_t i = 0; i < config->gpio_range_count; i++) {
		if ((strcmp(gpio->config->name,
		    ...
		    (gpio_pin < config->gpio_range_data[i].npins)) {
			if (pin != NULL) {
					 ...
					 u32_t *config)
{
	/**
	 * @code{.cogeno.py}
	 * cogeno.out('return {}(dev, pin, config);'.format(config_get))
	 * @endcode{.cogeno.py}
	 */
	/** @code{.cogeno.ins}@endcode */
}
					 ...
					 u32_t config)
{
	/**
	 * @code{.cogeno.py}
	 * cogeno.out('return {}(dev, pin, config);'.format(config_set))
	 * @endcode{.cogeno.py}
	 */
	/** @code{.cogeno.ins}@endcode */
}
				   ...
				   const char *owner)
{
	...
	if (pin >= pinctrl_tmpl_device_config_info(dev)->pin_count) {
		LOG_ERR("Invalid pin %d of %s on mux request by %s.",
	...
	if (current_owner != owner) {
		LOG_WRN("Pin %d of %s owned by %s but mux requested by %s.",
				...
				const char *owner)
{
	...
	current_owner = pinctrl_tmpl_mux_owner_get(dev, pin);
	if (current_owner != owner) {
		LOG_WRN("Pin %d of %s owned by %s but mux free tried by %s.",
			(int)pin, dev->config->name, current_owner, owner);
		return -EACCES;
	}
...
/**
 * @code{.cogeno.py}
 * cogeno.outl('\treturn {}(dev, pin, func);'.format(mux_get))
 * @endcode{.cogeno.py}
 */
/** @code{.cogeno.ins}@endcode */
}
...
static int pinctrl_tmpl_state_set(struct device *dev, u16_t state)
{
	...
	LOG_DBG("Set '%s' state.",
		config->state_name_data[
			config->pinctrl_state_data[state].name].name);
...
static const struct pinctrl_driver_api pinctrl_tmpl_driver_api = {
	...
	.config = {
		/**
		 * @code{.cogeno.py}
		 * cogeno.outl('.get = {},'.format(config_get))
		 * cogeno.outl('.set = {},'.format(config_set))
		 * @endcode{.cogeno.py}
		 */
		/** @code{.cogeno.ins}@endcode */
#if CONFIG_PINCTRL_RUNTIME_DTS
	...
	.mux = {
		/**
		 * @code{.cogeno.py}
		 * cogeno.outl('.get = {},'.format(mux_get))
		 * @endcode{.cogeno.py}
		 */
		/** @code{.cogeno.ins}@endcode */
 ...
 * _device_prio = cogeno.config_property('PINCTRL_INIT_PRIORITY', 2)
 * _device_api = 'pinctrl_tmpl_driver_api'
 *
 * for _pin_controller in _pin_controllers:
 *     _device_id = _pin_controller.get_device_id()
 *     _driver_name = _pin_controller.get_property('label')
 ...
 *
 *     if cogeno.config_property('CONFIG_PINCTRL_RUNTIME_DTS', 0):
 *         #
 *         # mux_pinmap data
 ...
 *     if cogeno.config_property('CONFIG_PINCTRL_RUNTIME_DTS', 0) == 0:
